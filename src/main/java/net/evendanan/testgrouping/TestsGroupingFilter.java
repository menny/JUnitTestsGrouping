...
public class TestsGroupingFilter extends Filter {
  ...
  /**
   * Adds a {@link TestsGroupingFilter} to the runner with {@link TestClassHashingStrategy} hashing strategy.
   */
  public static void addTestsGroupingFilterWithSystemPropertiesData(Filterable testRunner,
                                                                    boolean failIfDataMissing) {
    addTestsGroupingFilterWithSystemPropertiesData(testRunner, new TestClassHashingStrategy(),
        failIfDataMissing);
  }
  ...
  public static void addTestsGroupingFilterWithSystemPropertiesData(Filterable testRunner,
    ...
    if (systemProperties.containsKey(TEST_GROUPS_COUNT_SYSTEM_PROPERTY_KEY) && systemProperties
        .containsKey(TEST_GROUP_TO_EXECUTE_SYSTEM_PROPERTY_KEY)) {
      int groupCount =
      ...
      throw new IllegalStateException(String
          .format(Locale.US, "Could not find '%s' and '%s' in System.properties!",
              TEST_GROUPS_COUNT_SYSTEM_PROPERTY_KEY, TEST_GROUP_TO_EXECUTE_SYSTEM_PROPERTY_KEY));
    }
  }
   ...
   * Adds a {@link TestsGroupingFilter} to the runner with {@link TestClassHashingStrategy} hashing strategy.
   */
  public static void addTestsGroupingFilterToRunner(Filterable testRunner, int groupCount,
                                                    int groupToExecute) {
    addTestsGroupingFilterToRunner(testRunner, new TestClassHashingStrategy(), groupCount,
        groupToExecute);
  }

  public static void addTestsGroupingFilterToRunner(Filterable testRunner,
                                                    HashingStrategy hashingStrategy, int groupCount,
                                                    int groupToExecute) {
    try {
  ...
  public String describe() {
    return String
        .format(Locale.US, "Execute tests from group %d (out of %d)", mGroupToExecute, mGroupCount);
  }
}
