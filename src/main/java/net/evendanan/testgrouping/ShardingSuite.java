...
public class ShardingSuite extends Suite {
  ...
  /**
   * The <code>ShardUsing</code> annotation specifies the {@link HashingStrategy} to be used
   * when sharding the test-classes identified by {@link ShardingSuite}.
   */
   ...
   */
  public ShardingSuite(final Class<?> klass, final RunnerBuilder builder) throws InitializationError {
    super(builder, klass, getClassesForShard(getAllTestClasses(klass), getHashingStrategyFromAnnotation(klass)));
  }

  private static Class<?>[] getClassesForShard(final Class<?>[] classes, final Class<? extends HashingStrategy> strategyClass) throws InitializationError {
    try {
      ...
      if (systemProperties.containsKey(TEST_GROUPS_COUNT_SYSTEM_PROPERTY_KEY) && systemProperties.containsKey(TEST_GROUP_TO_EXECUTE_SYSTEM_PROPERTY_KEY)) {
        groupCount = Integer.parseInt(systemProperties.getProperty(TEST_GROUPS_COUNT_SYSTEM_PROPERTY_KEY));
        groupToExecute = Integer.parseInt(systemProperties.getProperty(TEST_GROUP_TO_EXECUTE_SYSTEM_PROPERTY_KEY));
      } else {
        groupCount = 1;
        groupToExecute = 0;
      }
      ...
      for (final Class<?> aClass : classes) {
        final Description testDescription = Description.createTestDescription(aClass, "ClassToFilter");
        if (strategy.calculateHashFromDescription(testDescription, groupCount) == groupToExecute) {
  ...
  private static Class<? extends HashingStrategy> getHashingStrategyFromAnnotation(final Class<?> klass) throws InitializationError {
    ShardUsing annotation = klass.getAnnotation(ShardUsing.class);
    if (annotation == null) {
      throw new InitializationError(String.format(Locale.US, "class '%s' must have a ShardUsing annotation", klass.getName()));
    }
    return annotation.value();
  }

  /**
   * The container class has a {@link RunWith} annotation OR the class
   * has at least one method that passes the {@link #isTestMethod} check.
   */
   ...
   * The default implementation is checking:
   * if it's a Junit class - {@link #isJunit4Test(Class)}.
   * And is not a {@link Suite} runner.
   * And it's a non-abstract, public class.
   *
